...
function Extract-Method {
    ...
    if (-not $match.Success) { throw "Method $Name not found" }
    $start = $match.Index
    $pre = $start
    while ($pre -gt 0) {
        $prevNewline = $script:content.LastIndexOf("`n", [int][Math]::Max(0, $pre - 1))
        if ($prevNewline -lt 0) { break }
    ...
    if ($braceStart -lt 0) { throw "No body for $Name" }
    $depth = 0
    $end = -1
    ...
    if ($end -lt 0) { throw "Unmatched braces for $Name" }
